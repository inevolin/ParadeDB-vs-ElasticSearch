...
# Local Database Runner for ParadeDB and Elasticsearch
...
display_connection_info() {
    ...
    echo "  ParadeDB:    kubectl port-forward svc/paradedb-service 5432:5432"
    echo "  Elasticsearch: kubectl port-forward svc/elasticsearch-service 9200:9200"
    echo
    echo "Connection strings:"
    echo "  ParadeDB (psql):    psql 'postgresql://benchmark_user:benchmark_password_123@localhost:5432/benchmark_db'"
    echo "  ParadeDB (Python):  postgresql://benchmark_user:benchmark_password_123@localhost:5432/benchmark_db"
    echo "  Elasticsearch:      http://localhost:9200"
    echo
    echo "To stop the databases, run: kubectl delete deployments --all"
    echo "To clean up data: kubectl delete pvc paradedb-pvc elasticsearch-pvc"
}

# Main execution
print_info "Starting local databases for ParadeDB and Elasticsearch"
